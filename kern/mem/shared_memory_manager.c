...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	//TODO: [PROJECT MS3] [SHARING - KERNEL SIDE] createSharedObject()
		...
		//case1: same name
		uint32 SizeUHeap = USER_HEAP_MAX - USER_HEAP_START;
		//case3: exceeds heap size
		if (size > SizeUHeap){
			return E_NO_MEM;
		}

		int retIndex = get_share_object_ID(ownerID,shareName);
		if(retIndex >= 0){
		...
		//virtual_address = (uint32) (uint32*)virtual_address;

		if (index >= 0){
		//allocate ALL required space in physMem/4k
		//allocate_chunk(curenv->env_page_directory, virtual_address, size, PERM_WRITEABLE);

			struct FrameInfo *ptr_frame_info;
			//struct FrameInfo* ptr_frame_info = get_frame_info(curenv->env_page_directory, i,&ptr_page_table);

			  size = ROUNDUP(size,PAGE_SIZE);

			  uint32 rangeStart = (uint32)(uint32*)virtual_address;
			  uint32 rangeEnd = ROUNDUP(((uint32)(uint32*)virtual_address) + size, PAGE_SIZE);
			  int tempIndex = 0;
			  for(uint32 i = rangeStart; i < rangeEnd; i += PAGE_SIZE)
			  {
			  allocate_frame(&ptr_frame_info);
			  map_frame(curenv->env_page_directory,ptr_frame_info, i,PERM_WRITEABLE|PERM_USER);
			  add_frame_to_storage(allocatedObject->framesStorage,ptr_frame_info, tempIndex);
			  tempIndex++;
			  }
...
{
	struct Env* myenv = curenv; //The calling environment
	int retID = get_share_object_ID(ownerID, shareName);
	if (retID == E_SHARED_MEM_NOT_EXISTS)
	{
		return E_SHARED_MEM_NOT_EXISTS;
	}
	else
	{
		struct Share *allocatedObject = &shares[retID];
		int IndexOfFrame  = 0;
		uint32 rangeStart = (uint32)(uint32*)virtual_address;
		uint32 rangeEnd =  ROUNDUP(allocatedObject->size,PAGE_SIZE) + rangeStart;

	   for (uint32 i = rangeStart; i < rangeEnd; i+=PAGE_SIZE)
	   {
		 struct FrameInfo* frame = get_frame_from_storage( allocatedObject->framesStorage, IndexOfFrame);
		 if (allocatedObject->isWritable)
		 {
			map_frame(curenv->env_page_directory, frame,i,PERM_USER |PERM_WRITEABLE );
		 }
		 else{
			map_frame(curenv->env_page_directory, frame,i, PERM_USER);
		 }
			IndexOfFrame++;
	  }
	   allocatedObject->references++;
		return retID;
	}
	return 0;
}
