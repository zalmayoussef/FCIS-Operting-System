...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	struct Semaphore *allocatedObject ;
			if(get_semaphore_object_ID(ownerEnvID, semaphoreName)==E_SEMAPHORE_NOT_EXISTS){
				uint32 rt=allocate_semaphore_object(&allocatedObject);
				if(rt==E_NO_SEMAPHORE){
					return E_NO_SEMAPHORE;
				}
				else{
					semaphores[rt].ownerID=ownerEnvID;
					strcpy(semaphores[rt].name,semaphoreName);
					semaphores[rt].value=initialValue;
					return rt;
				}
			}
			else{
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int rt = get_semaphore_object_ID(ownerEnvID,semaphoreName);
		if(rt!=E_SEMAPHORE_NOT_EXISTS){
			semaphores[rt].value-=1 ;
			if(semaphores[rt].value<=-1){
				enqueue(&semaphores[rt].env_queue,myenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	struct Env* MyEnv;
		uint32 rt = get_semaphore_object_ID(ownerEnvID, semaphoreName);
		if(rt!=E_SEMAPHORE_NOT_EXISTS){
			semaphores[rt].value+=1 ;
			if(semaphores[rt].value<=0){
				MyEnv=dequeue(&semaphores[rt].env_queue);
